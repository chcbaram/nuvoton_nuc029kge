 ...
 * $Revision: 7 $
 * $Date: 16/10/25 4:25p $
 * @brief    NUC029xGE series Timer PWM Controller(Timer PWM) driver source file
 *
 * @note
...
uint32_t TPWM_ConfigOutputFreqAndDuty(TIMER_T *timer, uint32_t u32Frequency, uint32_t u32DutyCycle)
{
    ...
    if((timer == TIMER0) || (timer == TIMER1))
    {
        if(CLK->CLKSEL0 & CLK_CLKSEL0_PCLK0SEL_Msk)
            u32PWMClockFreq = (SystemCoreClock >> 1);
        else
            u32PWMClockFreq = SystemCoreClock;
...
{
    timer->PWMADCTS = TIMER_PWMADCTS_TRGEN_Msk | (u32Condition << TIMER_PWMADCTS_TRGSEL_Pos);
}
...
/**
  ...
  * @note       The default loading mdoe is period loading mode.
  */
...
/**
  * @brief      Enable Brake Pin De-bounce Function
  *
  * @param[in]  timer           The pointer of the specified Timer module. It could be TIMER0, TIMER1, TIMER2, TIMER3.
  ...
  * @return     None
  *
  * @details    This function is used to enable external brake pin detector de-bounce function.
  */
...
/**
  * @brief      Disable Brake Pin De-bounce Function
  *
  * @param[in]  timer       The pointer of the specified Timer module. It could be TIMER0, TIMER1, TIMER2, TIMER3.
  *
  * @return     None
  *
  * @details    This function is used to disable external brake pin detector de-bounce function.
  */
